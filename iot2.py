...
username = "x-sam@brainscode.com"
password = "notheraccount2R"
account_name = "lc0"

#this will create a device with this id - error if it already exists
device_id = "02-00-86-dd-3c-3e"
...
def main():
    ...
    o = get_observations(aid, device_id, "909c6ba7-600c-4219-869d-575ddd652792")
    print_observation_counts(o)
    #print(o['series'][0]['points'][-1])
    for device in o['series']:
        print 'device: ', device['deviceName']
        print 'last measure: ', device['points'][-1]['value']
        print 'average measure: ', sum([float(x.values()[1]) for x in device['points']])/len(device['points'])
        print 'max measure: ', max([float(x.values()[1]) for x in device['points']])
